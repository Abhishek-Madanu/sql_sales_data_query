...
--how many category of products we have?
...
select category,gender,count(*) as no_of_transactions from retail_sales group by 1,2 order by 1,2 desc;
...
select * from(select 
	...
	round(cast(avg(total_sale) as numeric),2) as avg_total_sale,
	rank() over(partition by extract(year from sale_date) order by avg(total_sale) desc) as monthly_rank
	from retail_sales
	group by 1,2) as t1 where monthly_rank=1;
